...
async function toggleQuizStatus(quizId) {
    try {
        ...
        if (doc.exists) {
            ...
            alert(`âœ… Quiz ${newStatus === 'active' ? 'activated' : 'deactivated'} successfully!`);
            loadQuizzes(); // Refresh the list
        }
    } catch (error) {
        console.error('Error toggling quiz status:', error);
        alert('âŒ Error updating quiz status. Please try again.');
    }
}
...
async function previewQuiz(quizId) {
    try {
        ...
        if (doc.exists) {
            ...
            let previewText = `ðŸ“‹ QUIZ PREVIEW\n\n`;
        ...
        alert('âŒ Error loading quiz preview.');
    }
}

// Duplicate quiz function
async function duplicateQuiz(quizId) {
    if (!confirm('ðŸ”„ Create a copy of this quiz?')) return;
    ...
    try {
        ...
        if (doc.exists) {
            ...
            alert(`âœ… Quiz duplicated successfully!\n\nNew Quiz: "${duplicatedQuiz.title}"\nID: ${newDocRef.id}`);
            loadQuizzes(); // Refresh the list
        }
    } catch (error) {
        console.error('Error duplicating quiz:', error);
        alert('âŒ Error duplicating quiz. Please try again.');
    }
}
...
async function exportQuiz(quizId) {
    try {
        ...
        if (doc.exists) {
            ...
            alert('âœ… Quiz exported successfully!');
        }
    } catch (error) {
        console.error('Error exporting quiz:', error);
        alert('âŒ Error exporting quiz. Please try again.');
    }
}
...
async function viewQuizStats(quizId) {
    try {
        ...
        let statsText = `ðŸ“Š QUIZ STATISTICS\n\n`;
        ...
        alert('âŒ Error loading quiz statistics.');
    }
}
...
async function createQuiz() {
    try {
        ...
        alert(`âŒ Error creating quiz: ${error.message}`);
    }
}
...
async function uploadJSON() {
    ...
    try {
        ...
        alert(`âœ… Quiz uploaded successfully!\n\nQuiz ID: ${docRef.id}\nTitle: ${quizData.title}\nQuestions: ${quizData.questions.length}`);
        ...
        alert(`âŒ Error uploading quiz: ${error.message}`);
    }
}
...
async function viewUser(userId) {
    try {
        ...
        alert('âŒ Error loading user details.');
    }
}
...
async function exportAllData() {
    if (!confirm('ðŸ“¥ Export all quizzes and user data?')) return;
    ...
    try {
        ...
        alert('âŒ Error exporting data. Please try again.');
    }
}

// Edit quiz (placeholder - can be enhanced)
async function editQuiz(quizId) {
    alert(`ðŸ”§ Edit functionality for Quiz ID: ${quizId}\n\nThis feature will be implemented in the next update.\n\nFor now, you can:\n- Duplicate the quiz\n- Delete and recreate\n- Export and modify JSON`);
}

// Delete quiz
async function deleteQuiz(quizId) {
    if (!confirm('âš ï¸ Are you sure you want to delete this quiz?\n\nThis action cannot be undone!')) return;
    ...
    try {
        ...
        alert('âŒ Error deleting quiz. Please try again.');
    }
}

// Clear all sample data (DANGEROUS)
async function clearAllData() {
    if (!confirm('âš ï¸ DANGER: This will delete ALL quizzes!\n\nThis action cannot be undone. Are you absolutely sure?')) return;
    if (!confirm('âš ï¸ FINAL WARNING: All quizzes will be permanently deleted!\n\nType "DELETE" in the next prompt to confirm.')) return;
    ...
    try {
        ...
        await batch.commit();
        alert('âœ… All quizzes have been deleted!');
        loadQuizzes();
    } catch (error) {
        console.error('Error clearing data:', error);
        alert('âŒ Error clearing data. Please try again.');
    }
}
...
async function bulkActivateQuizzes() {
    ...
    try {
        ...
        alert(`âœ… ${selectedQuizzes.size} quiz(es) activated successfully!`);
        clearSelection();
        loadQuizzes();
        
    } catch (error) {
        console.error('Error bulk activating quizzes:', error);
        alert('âŒ Error activating quizzes. Please try again.');
    }
}
...
async function bulkDeactivateQuizzes() {
    ...
    try {
        ...
        alert(`âœ… ${selectedQuizzes.size} quiz(es) deactivated successfully!`);
        clearSelection();
        loadQuizzes();
        
    } catch (error) {
        console.error('Error bulk deactivating quizzes:', error);
        alert('âŒ Error deactivating quizzes. Please try again.');
    }
}
...
async function bulkUpdateCategory() {
    ...
    try {
        ...
        alert(`âœ… Category updated for ${selectedQuizzes.size} quiz(es)!`);
        clearSelection();
        loadQuizzes();
        ...
        alert('âŒ Error updating category. Please try again.');
    }
}
...
async function bulkExportQuizzes() {
    ...
    try {
        ...
        alert(`âœ… ${selectedQuizzes.size} quiz(es) exported successfully!`);
        
    } catch (error) {
        console.error('Error bulk exporting quizzes:', error);
        alert('âŒ Error exporting quizzes. Please try again.');
    }
}
...
async function exportAllQuizzes() {
    ...
    try {
        ...
        alert(`âœ… All ${currentQuizzes.length} quizzes exported successfully!`);
...
async function bulkDeleteQuizzes() {
    ...
    if (!confirm(`âš ï¸ DELETE ${selectedQuizzes.size} selected quiz(es)?\n\nThis action cannot be undone!`)) return;
    ...
    try {
        ...
        alert(`âœ… ${selectedQuizzes.size} quiz(es) deleted successfully!`);
        clearSelection();
        loadQuizzes();
        
    } catch (error) {
        console.error('Error bulk deleting quizzes:', error);
        alert('âŒ Error deleting quizzes. Please try again.');
    }
}
...
async function clearAllQuizzes() {
    ...
    if (!confirm(`âš ï¸ DANGER: Delete ALL ${currentQuizzes.length} quizzes?\n\nThis will permanently delete every quiz in your database!\n\nThis action cannot be undone!`)) return;
    
    if (!confirm('âš ï¸ FINAL WARNING: ALL QUIZZES WILL BE DELETED!\n\nAre you absolutely certain?')) return;
    ...
    try {
        ...
        alert(`âœ… All ${snapshot.size} quizzes have been deleted!`);
        clearSelection();
        loadQuizzes();
        
    } catch (error) {
        console.error('Error clearing all quizzes:', error);
        alert('âŒ Error clearing quizzes. Please try again.');
    }
}
...
console.log('âœ… Enhanced Admin Features loaded successfully!');
